...
def is_valid(current, test_value, nums):
    ...
    concatenated = int(f"{current}{nums[0]}")
    return is_valid(added, test_value, nums[1:]) or is_valid(multiplied, test_value, nums[1:]) or is_valid(concatenated, test_value, nums[1:])
